...
# ── Logica ELO (inlined da elo.py per compatibilità deploy) ───────────────────
...
def calculate_elo_deltas(players: list[dict]) -> dict[int, int]:
    """
    Calcola le variazioni ELO per tutti i giocatori dopo una partita.
    Confronto pairwise normalizzato; gestisce partite in coppia 2v2.

    players: [{"player_id": int, "elo": int, "total_score": int, "games_played": int}]
    Returns: {player_id: delta_elo}
    """
    ...
    for i in range(len(players)):
        for j in range(i + 1, len(players)):
            ...
            if teammate_of.get(pa["player_id"]) == pb["player_id"]:
                continue
            ea = _expected_score(pa["elo"], pb["elo"])
            eb = 1.0 - ea
            rank_a, rank_b = ranks[pa["player_id"]], ranks[pb["player_id"]]
            if rank_a < rank_b:
                sa, sb = 1.0, 0.0
            elif rank_a > rank_b:
                sa, sb = 0.0, 1.0
                ...
                sa, sb = 0.5, 0.5
            k_a = _get_k_factor(pa["games_played"])
            k_b = _get_k_factor(pb["games_played"])
...
class Database:
    ...
    async def get_head_to_head(self, player1_id: int, player2_id: int, limit: int = 20) -> list[dict]:
